...
export function useMangaGenerator() {
  ...
  const updateStepStatus = useCallback((stepId: string, status: GenerationStep['status'], data?: any, error?: string | undefined) => {
  ...
  const getUploadedCharacterReferences = useCallback((): UploadedCharacterReference[] => {
  ...
  const getUploadedSettingReferences = useCallback((): UploadedSettingReference[] => {
  ...
  const generateComic = async () => {
    ...
    // Reset all steps
    setGenerationSteps(steps => steps.map(step => ({ ...step, status: 'pending' as const, data: undefined, error: undefined })));
    ...
    try {
      ...
      if (!analysisResponse.ok) {
        throw new Error(await handleApiError(analysisResponse, 'Failed to analyze story'));
      }
      ...
      if (!charRefResponse.ok) {
        throw new Error(await handleApiError(charRefResponse, 'Failed to generate character references'));
      }

      const { characterReferences: charRefs } = await charRefResponse.json();
      setCharacterReferences(charRefs);
      updateStepStatus('characters', 'completed', { designs: charRefs.map(char => ({ name: char.name, url: char.image })) });
      ...
      if (!storyBreakdownResponse.ok) {
        throw new Error(await handleApiError(storyBreakdownResponse, 'Failed to break down story'));
      }
      ...
      for (let i = 0; i < breakdown.panels.length; i++) {
        ...
        setCurrentStepText(`Generating panel ${i + 1}/${breakdown.panels.length}...`);
        ...
        if (!panelResponse.ok) {
          ...
          trackError('panel_generation_failed', `Panel ${i + 1}: ${errorMessage}`);
      ...
      updateStepStatus('panels', 'completed', { panels: panels.map(p => ({ number: p.panelNumber, url: p.image })) });
      ...
      try {
        ...
        if (savedState) {
          ...
          if (savedState.characterReferences.length > 0) {
            updateStepStatus('characters', 'completed', { designs: savedState.characterReferences.map((char: any) => ({ name: char.name, url: char.image })) });
          }
          if (savedState.storyBreakdown) {
            updateStepStatus('layout', 'completed', { panels: savedState.storyBreakdown.panels });
          }
          if (savedState.generatedPanels.length > 0) {
            updateStepStatus('panels', 'completed', { panels: savedState.generatedPanels.map(p => ({ number: p.panelNumber, url: p.image })) });
            updateStepStatus('share', 'completed', { available: true });
          }
        }
      } catch (error) {
  ...
  }, [story, appStyle, storyAnalysis, storyBreakdown, characterReferences, generatedPanels, characterImages, settingImages, isLoadingState, getUploadedCharacterReferences, getUploadedSettingReferences]);
