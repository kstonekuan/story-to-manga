...
import type { AppStyle, UploadedImage } from "@/types/app";
...
function DropZone({ onUpload, children }: DropZoneProps) {
...
function ImageCard({ image, onRemove, onNameChange }: ImageCardProps) {
	return (
...
export function ImageUploader({
	...
	const handleNameChange = (
		...
		if (category === "character") {
			onCharacterImagesChange(
				characterImages.map((img) => (img.id === id ? { ...img, name } : img)),
			);
		} else {
			onSettingImagesChange(
				settingImages.map((img) => (img.id === id ? { ...img, name } : img)),
			);
		}
	};
		...
		<div className="space-y-6">
			...
			<div className="bg-blue-100 border-3 border-blue-500 p-3 transform -rotate-1 shadow-[3px_3px_0px_0px_#000]">
				<p className="text-sm font-bold text-blue-800 text-center">
					📸 Upload images to guide AI generation! Better references = Better
					results!
				</p>
			</div>
							...
							<div className="flex items-center gap-3">
								...
								<div className="text-left">
									...
									<div className="text-sm font-bold text-black opacity-80">
										{characterImages.length} image
										{characterImages.length !== 1 ? "s" : ""} ready
									</div>
								</div>
							</div>
							<ChevronDown
								className={`w-6 h-6 text-black transition-transform ${characterOpen ? "rotate-180" : ""}`}
							/>
						</div>
					</button>
									...
									{characterImages.map((image) => (
										<ImageCard
											...
											onNameChange={(id, name) =>
												handleNameChange(id, name, "character")
											}
											appStyle={appStyle}
										/>
									))}
						...
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-3">
								...
								<div className="text-left">
									...
									<div className="text-sm font-bold text-black opacity-80">
										{settingImages.length} image
										{settingImages.length !== 1 ? "s" : ""} ready
									</div>
								</div>
							</div>
							<ChevronDown
								className={`w-6 h-6 text-black transition-transform ${settingOpen ? "rotate-180" : ""}`}
							/>
						</div>
					</button>
								...
								<div className="grid grid-cols-2 md:grid-cols-3 gap-3">
									{settingImages.map((image) => (
										<ImageCard
											...
											onNameChange={(id, name) =>
												handleNameChange(id, name, "setting")
											}
											appStyle={appStyle}
										/>
									))}
